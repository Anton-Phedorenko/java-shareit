...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    @Query("SELECT b FROM Booking b JOIN Item i ON i.id = b.item.id " +
            "WHERE i.owner.id = ?1")
    List<Booking> findAllByOwnerAll(Long ownerId, Pageable pageable);
    ...
    List<Booking> findAllByOwnerFuture(Long ownerId, Pageable pageable);
    ...
    List<Booking> findAllByOwnerWaiting(Long ownerId, Pageable pageable);

    @Query("SELECT b FROM Booking b JOIN Item i ON i.id = b.item.id " +
            "WHERE i.owner.id = ?1 AND (b.status = 'REJECTED' OR b.status = 'CANCELED')")
    List<Booking> findAllByOwnerRejected(Long ownerId, Pageable pageable);

    @Query("SELECT b FROM Booking b " +
            "WHERE b.booker.id = ?1")
    List<Booking> findAllByBookerAll(Long bookerId, Pageable pageable);
    ...
    List<Booking> findAllByBookerFuture(Long bookerId, Pageable pageable);
    ...
    List<Booking> findAllByBookerWaiting(Long bookerId, Pageable pageable);

    @Query("SELECT b FROM Booking b " +
            "WHERE b.booker.id = ?1 AND (b.status = 'REJECTED' OR b.status = 'CANCELED')")
    List<Booking> findAllByBookerRejected(Long bookerId, Pageable pageable);
